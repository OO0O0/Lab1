...
public class gui  {
    ...
    static void DrawGraph(Graph G) {
        EdgeNode p = new EdgeNode();
		GraphViz gViz=new GraphViz( ".",".\\release\\bin\\dot.exe");
        gViz.start_graph();
        for (int i = 0; i < G.vertex_num; i++) {
            //System.out.print(G.node_list[i].date);
    ...
    public static String generateNewText(Graph G, String inputText) {
        ...
        //System.out.println(inputText_split.length);
    ...
    {
        EdgeNode p=new EdgeNode();
        GraphViz gViz=new GraphViz( ".",".\\release\\bin\\dot.exe","temp");
        gViz.start_graph();
    ...
    public void actionPerformed(ActionEvent event) {
    	...
    	if(source == btnload) {
            try {
                ...
                if (f.length()>0 && f.length()<65536){
                    ...
                    gui.str1 = (gui.str1.replaceAll("[^(A-Xa-z)]", " ")).replaceAll(" +", " ");
		...
		*/
    	...
    	if(source == btn25) {
    		...
    		GUIPanel.out1.setText(tmp); 
    		JOptionPane.showMessageDialog(gui.frame,"The random path is: "+GUIPanel.out1.getText());
    	}
